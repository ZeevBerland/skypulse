...
export function buildRecommendation(
  ...
  return {
    ...
    owner: 'store_manager',
    valid_from: `${date}T${/^\d{2}:\d{2}$/.test(raw.time_window?.split('-')[0] ?? '') ? raw.time_window.split('-')[0] : '08:00'}:00`,
    valid_until: `${date}T${/^\d{2}:\d{2}$/.test(raw.time_window?.split('-')[1] ?? '') ? raw.time_window.split('-')[1] : '22:00'}:00`,
